...
/**
 * <p>
 * This storage layer acts as a generic data store. For now, this storage layer is mostly used to persist/retrieve JSON
 * objects. Raw JSON objects are stored to a {@link BlobStore}. Indexed terms are stored to a {@link TermStore}.
 * </p>
 *
 * <p>
...
final public class DataStore implements AutoCloseable {
  ...
  public DataStore(String name, BlobStore blobStore, TermStore termStore, Cache cache,
      AbstractBlobProcessor blobProcessor, AbstractTermProcessor termProcessor, AbstractHashProcessor hashProcessor) {
    name_ = Preconditions.checkNotNull(name, "name should neither be null nor empty");
  ...
  public boolean grantWritePermissionOnTermStore(String username) {
    ...
    return Users.grantPermission(termStore_.configurations().connector(), username, termStoreName(name()),
        TablePermission.WRITE);
  }
  ...
  public boolean revokeWritePermissionOnTermStore(String username) {
    ...
    return Users.revokePermission(termStore_.configurations().connector(), username, termStoreName(name()),
        TablePermission.WRITE);
  }
  ...
  public boolean create() {
    ...
    @Var boolean isOk = blobStore_.create();
  ...
  public boolean remove(String dataset) {
    ...
    @Var boolean isOk = termStore_.removeDataset(dataset);
  ...
  public boolean persist(String dataset, String docId, Map<String, Object> json) {
    return persistJson(dataset, docId, JsonCodec.asString(json), key -> true, StringCodec::defaultTokenizer, true);
  }
  ...
  public boolean reindex(String dataset, String docId, Map<String, Object> json) {
    return persistJson(dataset, docId, JsonCodec.asString(json), key -> true, StringCodec::defaultTokenizer, false);
  }
   ...
   */
  public long termCardinalityEstimationForBuckets(Authorizations authorizations, String dataset, Set<String> fields,
      String term) {
  ...
  /**
   * Estimate the number of buckets with at least one of occurrence of all terms in {@code [minTerm, maxTerm]}.
   *
   * @param authorizations authorizations.
   * @param dataset        dataset (optional).
   * @param fields         which fields must be considered (optional).
  ...
  public long termCardinalityEstimationForBuckets(Authorizations authorizations, String dataset, Set<String> fields,
    ...
    return termStore_.termCardinalityEstimationForBuckets(authorizations, dataset, fields, minTerm, maxTerm)
        .reduce(0L, (carry, t) -> carry + t.count());
  }
   ...
   */
  public View<String> docsIdsSorted(Authorizations authorizations, String dataset, String term, Set<String> fields,
    ...
    return termsSortedByTermAndDocId(authorizations, dataset, term, fields, docsIds).groupSorted(
        (t1, t2) -> t1.term().equals(t2.term())).reduce(CARRY, (carry, termz) -> {
  ...
  public View<Term> termsSortedByTermAndDocId(Authorizations authorizations, String dataset, String term,
  ...
  /**
   * Get the ids of all documents where at least one token matches a term in {@code [minTerm, maxTerm]} (ordered).
   *
   * @param authorizations authorizations.
   * @param dataset        dataset (optional).
   * @param fields         which fields must be considered (optional).
   ...
   * @return an ordered stream of documents ids.
   */
  public View<String> docsIdsSorted(Authorizations authorizations, String dataset, Set<String> fields, Object minTerm,
    ...
    View<String> CARRY = View.of();
    return termsSortedByTermAndDocId(authorizations, dataset, fields, minTerm, maxTerm, docsIds).groupSorted(
        (t1, t2) -> t1.term().equals(t2.term())).reduce(CARRY, (carry, termz) -> {
  ...
  public View<Term> termsSortedByTermAndDocId(Authorizations authorizations, String dataset, Set<String> fields,
      Object minTerm, Object maxTerm, BloomFilters<String> docsIds) {
    return termStore_.termsSortedByTermAndBucketId(authorizations, dataset, fields, minTerm, maxTerm, docsIds);
  }
   ...
   */
  public View<Term> terms(Authorizations authorizations, String dataset, String term, Set<String> fields,
      BloomFilters<String> docsIds) {
  ...
  /**
   * Get the ids of all documents where at least one token matches a term in {@code [minTerm, maxTerm]} (unordered).
   *
   * @param authorizations authorizations.
   * @param dataset        dataset (optional).
   * @param fields         which fields must be considered (optional).
   ...
   * @return an ordered stream of documents ids.
   */
  public View<String> matchValueSortedByDocId(Authorizations authorizations, String dataset, String field,
      Object value) {
    ...
    return readHash(termStore_.scanner(authorizations), dataset, field, MaskingIterator.hash(null, value.toString()));
  }
   ...
   */
  public View<String> matchValue(Authorizations authorizations, String dataset, String field, Object value) {
   ...
   * @return an ordered stream of documents ids.
   */
  public View<String> matchHashSortedByDocId(Authorizations authorizations, String dataset, String field, String hash) {
  ...
  /**
   * Get the ids of all documents where a field hashed value exactly matches a given hash (unordered).
   *
   * @param authorizations authorizations.
   * @param dataset        dataset.
   * @param field          which field must be considered.
   ...
   * @return an unordered stream of documents ids.
   */
  public View<String> matchHash(Authorizations authorizations, String dataset, String field, String hash) {
    ...
    return readHash(termStore_.batchScanner(authorizations, NB_QUERY_THREADS), dataset, field, hash);
  }
   ...
   * @param keepField  filter applied on all JSON attributes before value tokenization (optional). This predicate should
   *                   return true iif the field's value must be indexed.
   ...
   * @return true if the operation succeeded, false otherwise.
   */
  private boolean persistJson(String dataset, String docId, String json, Predicate<String> keepField,
      Function<String, SpanSequence> tokenizer, boolean jsonAsBlob) {
    ...
    Map<String, Object> newJson = new JsonFlattener(json).withSeparator(SEPARATOR_CURRENCY_SIGN).flattenAsMap();
    ...
    for (String field : newJson.keySet()) {
      ...
      @Var Object value = newJson.get(field);
      ...
      if (!persistHash(dataset, docId, newField, value)) {
        logger_.error(LogFormatter.create(true)
            .message(String.format("Persistence of the hash for field %s failed for document %s.", field, docId))
            .formatError());
        return false;
      }
    ...
    for (Map.Entry<String, Multiset<Object>> field : fields.entrySet()) {
      for (Multiset.Entry<Object> term : field.getValue().entrySet()) {
        isOk = isOk && persistTerm(dataset, docId, field.getKey(), term.getElement(), term.getCount());
      }
    }
    return isOk;
  }
  ...
  private boolean persistTerm(String dataset, String docId, String field, Object term, int nbOccurrencesInDoc) {
    return termProcessor_ == null || termProcessor_.write(dataset, docId, field, term, nbOccurrencesInDoc);
  }
  ...
  private View<String> readHash(ScannerBase scanner, String dataset, String field, String hash) {
    ...
    if (field != null) {
      ...
      IteratorSetting setting = new IteratorSetting(31, "TermStoreBucketFieldFilter", TermStoreBucketFieldFilter.class);
