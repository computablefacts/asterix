...
public final class GoldLabel {
   ...
   */
  public static void save(File file, View<GoldLabel> goldLabels) {
  ...
  public static GoldLabel fromFact(Fact fact) {
    ...
    Preconditions.checkArgument(fact.isVerified(), "fact must be verified");
    Preconditions.checkNotNull(fact.provenance(), "provenance should not be null");
    Preconditions.checkNotNull(fact.provenance().string(), "string should not be null");

    String id = Integer.toString(fact.id_, 10);
    String label = fact.type();
    String data = fact.provenance().string();
    boolean isTrueNegative = fact.isRejected();
    boolean isTruePositive = fact.isAccepted();
    boolean isFalseNegative = false;
    boolean isFalsePositive = false;

    return new GoldLabel(id, label, data, isTrueNegative, isTruePositive, isFalseNegative, isFalsePositive);
  }
