...
final public class SnippetExtractor {
   ...
   * @return the snippet.
   */
  public static String extract(List<String> words, String text, int relLength, int prevCount, @Var String indicator) {
    ...
    @Var int startPos = locations == null || locations.isEmpty() ? 0 : snippetLocation(locations, relLength, prevCount);
  ...
  public static int snippetLocation(List<Span> locations, int relLength, int prevCount) {
    ...
    for (int i = 0; i < locations.size(); i++) {
      ...
      for (int j = i + 1; j < locations.size() && (locations.get(j).begin() - beginPos) < relLength; j++) {
        endPos = locations.get(j).begin() + locations.get(j).text().length();
        words.add(locations.get(j).text());
      }

      if (words.size() > nbDistinctWords /* maximize the number of distinct words */ || (words.size() == nbDistinctWords
