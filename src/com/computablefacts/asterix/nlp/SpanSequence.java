...
final public class SpanSequence implements Iterable<Span>, Comparable<SpanSequence> {
  ...
  public Span span(int index) {
    ...
    Preconditions.checkArgument(0 <= index && index < sequence_.size(), "index should be such as 0 <= index < %s",
        size());
  ...
  public SpanSequence sequence(int min, int max) {
    ...
    Preconditions.checkArgument(0 <= min && min <= sequence_.size(), "min should be such as 0 <= min <= %s", size());
    Preconditions.checkArgument(0 <= max && max <= sequence_.size(), "max should be such as 0 <= max <= %s", size());
