...
public class ModelTest {
  ...
  public void testCallCommandLine() throws Exception {
    ...
    DocumentTest.papers().index().flatten(doc -> View.of(
                Splitter.on('\f').trimResults().omitEmptyStrings().split((String) doc.getValue().text()))
            .map(page -> {
              boolean isTruePositive = Pattern.matches("(?ms).*crowdsourcing.*", page.toLowerCase());
              return new GoldLabel(doc.getKey().toString(), "crowdsourcing",
                  (String) doc.getValue().text(), !isTruePositive, isTruePositive, false, false);
            })).index().filter(
            item -> item.getValue().isTruePositive() || item.getKey() % 4 == 0 /* speed-up the test */)
        .map(Map.Entry::getValue)
        .toFile(goldLabel -> JsonCodec.asString(goldLabel.asMap()), goldLabels, false, true);

    // crowdsourcing
    String[] args = new String[]{documents.getAbsolutePath(), goldLabels.getAbsolutePath(),
        "crowdsourcing", "dt,svm,logit"};
    Model.main(args);

    Assert.assertTrue(
        new File(path + File.separator + "ensemble-model-crowdsourcing.xml.gz").exists());
